...
import nook.test.market_nookat.ui.fragment.profile.active.ActiveFragment.Companion.IOP
...
class AdsActiveFragment : BaseFragment<AdsActiveViewModel,FragmentAdsActiveBinding>() {

    override val viewModel: AdsActiveViewModel by viewModel()
    override fun inflateViewBinding(inflater: LayoutInflater, container: ViewGroup?, boolean: Boolean): FragmentAdsActiveBinding {
        return FragmentAdsActiveBinding.inflate(inflater,container,false)
    }
    ...
    private fun onViewModel() {
        ...
        if (arguments != null) {
            model = arguments?.getSerializable(IOP) as Model
                ...
                viewModel.getOneAds(model.img).collect { data ->
                    when (data.status) {
                        Status.SUCCESS -> {
                            ...
                            val adapter = data.data?.data?.photos?.let { CustomSliderAdapter(it, this@AdsActiveFragment::onClick) }
                            if (adapter != null) {
                                binding.vpPhoto.setSliderAdapter(adapter)
                            }

                            binding.btnEdit.setOnClickListener {
                                findNavController().navigate(R.id.editActiveFragment, bundleOf(KO to data.data?.data))
                            }
                        }
    ...
    private fun onClick(pos: Int){
        ...
        findNavController().navigate(R.id.photoActiveFragment, bundleOf(WET to fgh))
    }
    companion object {
        const val KO = "bghkk"
        const val WET = "awergv"
    }
